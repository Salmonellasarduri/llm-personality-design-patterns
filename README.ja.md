...
人格ファイルは「望ましい性格」を記述できます。しかし、時間の中で実際に起きることまでは統治できませんでした。そこで必要になったのが、人格・記憶・対話・変更検証の境界を明確に分けることです。このリポジトリは、その境界を抽出したものです。なお、ここで扱う事故はシステムの応答挙動であり、意識や主観的な苦痛を主張するものではありません。

これは人格プロンプト集ではありません。完成済みの総合フレームワークでもありません。必要な部分から個別に導入できる、運用由来の参照構造です。
...
一回限りのキャラクターであれば、人格テンプレートやsystem promptの方が直接的です。このリポジトリが始まるのは、記憶と経験が増え続けても、数週間から数か月にわたって同じ存在として認識される必要がある地点です。実在人物のDigital Twin生成や、導入するだけで動く総合基盤は対象外です。
